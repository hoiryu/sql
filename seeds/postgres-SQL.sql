...
-- 고객 테이블 생성
CREATE TABLE users (
    user_id BIGSERIAL,          -- MySQL: BIGINT AUTO_INCREMENT → PostgreSQL: BIGSERIAL
    name VARCHAR(255) NOT NULL,
    email VARCHAR(255) NOT NULL UNIQUE,
    address VARCHAR(255),
    birth_date DATE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,  -- MySQL: DATETIME → PostgreSQL: TIMESTAMP
    PRIMARY KEY (user_id)
);

-- 상품 테이블 생성
CREATE TABLE products (
    product_id BIGSERIAL,       -- MySQL: BIGINT AUTO_INCREMENT → PostgreSQL: BIGSERIAL
    name VARCHAR(255) NOT NULL,
...
-- 주문 테이블 생성
CREATE TABLE orders (
    order_id BIGSERIAL,         -- MySQL: BIGINT AUTO_INCREMENT → PostgreSQL: BIGSERIAL
    user_id BIGINT NOT NULL,
    product_id BIGINT NOT NULL,
    order_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP,  -- MySQL: DATETIME → PostgreSQL: TIMESTAMP
...
CREATE TABLE employees (
    employee_id BIGSERIAL,      -- MySQL: BIGINT AUTO_INCREMENT → PostgreSQL: BIGSERIAL
    name VARCHAR(255) NOT NULL,
    manager_id BIGINT,
...
CREATE TABLE sellers (
    seller_id SERIAL PRIMARY KEY,   -- MySQL: INT PRIMARY KEY AUTO_INCREMENT → PostgreSQL: SERIAL PRIMARY KEY
...
-- 상품 테이블 생성
CREATE TABLE items (
    item_id SERIAL PRIMARY KEY,     -- MySQL: INT PRIMARY KEY AUTO_INCREMENT → PostgreSQL: SERIAL PRIMARY KEY
...
-- CASCADE 동작 확인: 션 삭제 시 관련 주문도 함께 삭제됨
...
CREATE TABLE products (
    product_id BIGSERIAL PRIMARY KEY,   -- MySQL: BIGINT AUTO_INCREMENT → PostgreSQL: BIGSERIAL
    name VARCHAR(255) NOT NULL,
    category VARCHAR(100),
...
CREATE TABLE logs (
    id SERIAL PRIMARY KEY,          -- MySQL: INT AUTO_INCREMENT → PostgreSQL: SERIAL
...
-- PostgreSQL은 DELIMITER를 사용하지 않으며, 프로시저 바디를 $$ 로 감싼다
CREATE OR REPLACE PROCEDURE sp_change_user_address(
    user_id_param INT,              -- MySQL: IN 키워드 → PostgreSQL: 생략 가능 (기본값이 IN)
    new_address_param VARCHAR(255)
)
LANGUAGE plpgsql                   -- PostgreSQL: 사용할 언어를 명시해야 함
AS $$
BEGIN
...
CREATE TABLE stored_items (
    item_id BIGSERIAL PRIMARY KEY,  -- MySQL: BIGINT AUTO_INCREMENT → PostgreSQL: BIGSERIAL
    name VARCHAR(255) NOT NULL,
    price INT NOT NULL,
...
-- PostgreSQL은 DELIMITER를 사용하지 않으며, 함수 바디를 $$ 로 감싼다
CREATE OR REPLACE FUNCTION fn_get_final_price(
    price_param INT,
    discount_rate_param DECIMAL(5, 2)
)
RETURNS DECIMAL(10, 2)
IMMUTABLE                          -- MySQL: DETERMINISTIC → PostgreSQL: IMMUTABLE (입력이 같으면 결과가 항상 같음)
LANGUAGE plpgsql                   -- PostgreSQL: 사용할 언어를 명시해야 함
AS $$
BEGIN
...
-- PostgreSQL은 트리거 로직을 별도의 트리거 함수로 먼저 만든 후, CREATE TRIGGER로 연결한다

-- 1단계: 트리거가 실행할 함수 생성
CREATE OR REPLACE FUNCTION fn_backup_user()
RETURNS TRIGGER                    -- 트리거 함수는 반드시 TRIGGER 타입을 반환해야 함
LANGUAGE plpgsql
AS $$
BEGIN
    INSERT INTO retired_users (id, name, email, retired_date)
    VALUES (OLD.user_id, OLD.name, OLD.email, CURRENT_DATE);  -- MySQL: CURDATE() → PostgreSQL: CURRENT_DATE
    RETURN OLD;                    -- BEFORE 트리거에서는 OLD(삭제될 행)를 반환해야 삭제가 진행됨
END;
$$;

-- 2단계: 트리거 함수를 users 테이블의 DELETE 이벤트에 연결
DROP TRIGGER IF EXISTS trg_backup_user ON users;  -- 재실행을 위해 기존 트리거 삭제
CREATE TRIGGER trg_backup_user
BEFORE DELETE ON users
FOR EACH ROW EXECUTE FUNCTION fn_backup_user();   -- MySQL: BEGIN...END 인라인 → PostgreSQL: EXECUTE FUNCTION 으로 연결
